...
不需要在服务端保存会话信息，特别适用于分布式微服务。
    我的总结：1.节省服务端空间 2.防止跨站请求攻击3.避免第三方缓存组件来实现session共享
```
...
```java
# 3.Payload - 令牌的第二部分是有效负载，其中包含声明。声明是有关实体（通常是用户）和其他数据的声明。同样的，它会使用 Base64 编码组成 JWT 结构的第二部分
```
...
//https://blog.csdn.net/unique_perfect/article/details/109219826
```
...
```java
git@gitee.com:AugustT2/springboot-jwt-2020.git
```
